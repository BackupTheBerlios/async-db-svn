...
class statement_impl : private boost::noncopyable
{
public:
...
//    statement_impl(prepare_temp_type const &prep);
    ...
    void prepare( std::string const &query, eStatementType eType = eRepeatableQuery );
...
private:
    ...
    db::details::statement_backend *backEnd_;
};
